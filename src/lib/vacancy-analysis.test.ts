...
describe("vacancy analysis", () => {
  ...
  it("recognizes a junior frontend job without labelling it as Ausbildung", () => {
    const text = "Ein Berliner SaaS-Start-up sucht einen Junior Frontend Developer (m/w/d).";

    expect(extractRole(text, "fallback", "de")).toBe("Junior Frontend Developer (m/w/d)");
    expect(extractRole(text, "fallback", "en")).toBe("Junior Frontend Developer (m/f/d)");
  });
});
